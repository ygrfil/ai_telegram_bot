...
def get_provider(provider_name: str, config: Config) -> BaseAIProvider:
    ...
    elif provider_name == "gemini" or provider_type == "google":
        provider = GeminiProvider(config.gemini_api_key, config=config)
    elif provider_type in ["openai", "anthropic", "perplexity"] or provider_name == "openrouter":
